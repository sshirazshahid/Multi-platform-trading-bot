...
def test_funding_books_every_realized_settlement_exactly_once(wh, monkeypatch):
    ...
    (a 4h base settles twice inside one 8h window) and never books the live
    print. Hermetic: the settlement source is stubbed, never data/."""
    closes = _dip_series()
    probe, p = _entered(wh, closes, "ZfadeProbeAgent")
    row = wh.query(
        "SELECT proposal_id, signal_bar_ts FROM shadow_bundle_mr_probe")[0]
    pid, entry = row["proposal_id"], float(row["signal_bar_ts"])
    book = _stub_settlements(monkeypatch, entry)

    def _sum():
        return wh.query(
            "SELECT realized_funding_rate_sum s FROM shadow_bundle_mr_probe "
