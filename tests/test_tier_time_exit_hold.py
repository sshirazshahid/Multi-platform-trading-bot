...
def test_horizon_zombie_protection(monkeypatch):
    monkeypatch.setitem(TIER_GEOMETRY_TIME_EXIT_HOLD, "enabled", True)
    monkeypatch.setitem(TIER_GEOMETRY_TIME_EXIT_HOLD, "max_hold_hours", 72.0)
...
def test_compressed_geometry_never_holds(monkeypatch):
...
def test_missing_tp_sentinel_never_holds(monkeypatch):
    monkeypatch.setitem(TIER_GEOMETRY_TIME_EXIT_HOLD, "enabled", True)
...
def test_inverted_barriers_never_hold(monkeypatch):
