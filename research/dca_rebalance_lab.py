...
def simulate_lump_sum(prices, capital=1000.0, fee_pct=DEFAULT_FEE_PCT,
                      slippage_pct=DEFAULT_SLIPPAGE_PCT,
                      periods_per_year=365) -> SimResult:
...
def simulate_dca(prices, capital=1000.0, interval_bars=1, fee_pct=DEFAULT_FEE_PCT,
                 slippage_pct=DEFAULT_SLIPPAGE_PCT, dip_threshold=None,
                 dip_multiplier=1.0, periods_per_year=365) -> SimResult:
    ...
    for i, price in enumerate(prices):
        if i in buy_set and cash > 1e-12:
            amount = base_amount
            if (dip_threshold is not None and last_buy_price is not None
    ...
    return SimResult(
        ...
        fees_paid=fees_paid,
        equity_curve=curve,
        extra={"coins": coins, "avg_cost": avg_cost, "uninvested_cash": cash,
               "n_buys": n_trades},
    )
...
def simulate_buy_and_hold_basket(price_series, targets, capital=1000.0,
    ...
    curve = [_basket_value(coins, {a: price_series[a][i] for a in targets})
             for i in range(n)]
    return SimResult(
        label="basket_hold",
        invested=capital,
...
def simulate_threshold_rebalance(price_series, targets, capital=1000.0,
                                 threshold=0.05, fee_pct=DEFAULT_FEE_PCT,
                                 slippage_pct=DEFAULT_SLIPPAGE_PCT,
                                 periods_per_year=365) -> SimResult:
    ...
    for i in range(n):
        ...
        if max(abs(d) for d in drift.values()) > threshold:
            ...
            traded_notional = sum(
                abs(targets[a] * total - coins[a] * px[a]) for a in targets) / 2.0
...
def compare_single_asset(prices, capital=1000.0, interval_bars=1,
                         periods_per_year=365, **kw) -> dict:
    ...
    return {
        ...
        "dca": simulate_dca(
            prices, capital=capital, interval_bars=interval_bars,
            periods_per_year=periods_per_year, **kw),
    }


def _fmt(r: SimResult) -> str:
    return (f"  {r.label:<20} return={r.total_return_pct:+7.2f}%  "
...
if __name__ == "__main__":
    ...
    print(_fmt(simulate_threshold_rebalance(series, tgt, capital=1000.0,
                                            threshold=0.05)))
