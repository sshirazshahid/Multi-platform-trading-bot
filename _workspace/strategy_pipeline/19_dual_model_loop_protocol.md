...
1. **"Implement" means log-only.** Agreed strategies are implemented ONLY as log-only shadow probes (`core/agents/`) or pre-registered screens through `strategy-evidence-pipeline`. The loop makes ZERO live/paper-order decision-path changes. Promotion beyond log-only = frozen gate (`core/promotion_gate.py`, ≥30 RESOLVED per arm) + explicit owner sign-off, always.
   ...
   **AI Reviewer (added 2026-07-26, owner directive — replaces the committee's "Human Reviewer" role for PAPER):** the `ai-reviewer` agent (Opus 5, `.claude/agents/ai-reviewer.md`) holds final APPROVE/REJECT authority over every PAPER-scope decision in this loop — S1 adjudications, S2 screen verdicts, S3 scout candidates, ledger rows, and log-only probe integrations. Its APPROVE is final; the loop does NOT wait for the owner on those. UNCHANGED and non-delegable: the frozen gate stays arithmetic (no reviewer argues past a failed threshold), promotion to ORDER FLOW stays owner-signed, and `docs/CONTROLLED_LIVE_CHECKLIST.md` is signable by the human alone. Money-scope items emit `ESCALATE_TO_HUMAN`.
...
**Cadence (restructured 2026-07-23 for cost):** hourly ScheduleWakeup ticks were burning ~$10–20 each on a 330k-token context (~$194 first session-day) — indefensible against the owner's ~$420 capital. Current architecture: (a) persistent funnel/health **Monitor** (task bc9xjh9za) = instant wake on floor-crossings ≥30, profile regression, halt, incident latch; (b) **daily cron** (job 4cfb200d, 09:07 local, session-only, auto-expires 7 days) = the one scheduled iteration/day; (c) ScheduleWakeup only while a multi-step stage is actively in flight — never as an idle heartbeat. If this background session ends, cron+monitor die with it — re-arm via /loop, or ask for a durable Windows schtask running `claude -p` if the owner wants it restart-proof.
...
- **S3 Scout (≤1 per UTC day, ONLY if S1/S2 produced no work):** parallel scout — Fable web sweep + `codex exec` scout on the same brief (delta since last sweep only, max 2 candidates each). A candidate enters the prereg queue only if BOTH models rate it screen-worthy AND it passes the ledger reopen check. Most days produce nothing; that is correct behavior, not failure.
  ...
  **Investment committee packaging (added 2026-07-25, Option A integrate):** on S3 days the orchestrator MAY run the `investment-committee` skill (bull-researcher → bear-researcher → debate-engine → optional final-verdict) to package a candidate before it enters the prereg queue. See `docs/AI_INVESTMENT_COMMITTEE.md` and `.claude/skills/investment-committee/SKILL.md`. Screen/backtest/install rules are **unchanged**: hand off to `strategy-evidence-pipeline`; both-agree; log-only implement; frozen gate + owner sign-off for promotion. Committee APPROVE ≠ live or paper OPEN authority. Do not install Jesse/OpenBB/Qlib/TradingAgents/TensorTrade.
