...
def walk_forward(
    strat_by_param: dict, *, is_days: int, oos_days: int, step: int, objective: str = "sharpe"
) -> list[dict]:
...
def _run_wfa(strat_by_param, ret_full, *, is_days, oos_days, step, objective):
    folds = walk_forward(
        strat_by_param, is_days=is_days, oos_days=oos_days, step=step, objective=objective
    )
    if not folds:
        return None
    ...
    return {
        ...
        "pct_folds_positive": pos_folds / nf,
        "pct_folds_beat_ret": beat_ret / nf,
        "pct_folds_beat_sharpe": beat_sr / nf,
        "n_distinct_params": len(set(params)),
        "modal_param": Counter(params).most_common(1)[0],
...
def main() -> int:
    ...
    primary = _run_wfa(
        strat_by_param, ret_full, is_days=504, oos_days=126, step=126, objective="sharpe"
    )
    # robustness sweep
    sweep = []
    for objective in ("sharpe", "calmar"):
        for isd, oosd in ((504, 126), (756, 126), (252, 63)):
            r = _run_wfa(
                strat_by_param, ret_full, is_days=isd, oos_days=oosd, step=oosd, objective=objective
            )
            if r:
                sweep.append(r)
    ...
    L = [
        f"# Nifty 50 SMA crossover — WALK-FORWARD study — {today}",
        "",
    ...
    for r in sweep:
        L.append(
            f"| {r['objective']} | {r['is_days']}/{r['oos_days']} | {r['n_folds']} | "
    ...
    L += [
        "",
        "## Per-fold (primary)",
        "| fold | OOS dates | param | IS Sharpe | OOS Sharpe |",
        "|---|---|---|---|---|",
    ]
    for i, f in enumerate(p["folds"], 1):
        L.append(
            f"| {i} | {dates[f['oos_lo']]}→{dates[min(f['oos_hi'] - 1, n - 1)]} | {f['param']} | "
    ...
    (REPORTS / f"nifty_sma_walkforward_{today}.json").write_text(
        json.dumps(
            {
                "date": today,
                "bars": n,
    ...
    print(
        f"  WFA OOS : total {w['total_return'] * 100:+.1f}%  Sharpe {w['sharpe']:.2f}  "
        f"MaxDD {w['max_drawdown'] * 100:.1f}%"
    )
    print(
        f"  B&H     : total {b['total_return'] * 100:+.1f}%  Sharpe {b['sharpe']:.2f}  "
        f"MaxDD {b['max_drawdown'] * 100:.1f}%"
    )
    print(
        f"  beats B&H ret/SR: {p['beats_bh_ret']}/{p['beats_bh_sharpe']} | WFE {p['wfe_median']:.2f} "
