...
DIV_YIELD = {
    "Nifty50": 1.3, "Sensex": 1.2, "S&P500": 1.98, "Nasdaq": 1.0,
    "Dow": 2.1, "FTSE100": 3.6, "Nikkei225": 1.7, "DAX": 0.0,
}
...
def main() -> int:
    ...
    for iname, sym in INDICES.items():
        ...
        for sname, (gen, grid) in STRATEGIES.items():
            ...
            r = _run_wfa(sbp, tr_ret, is_days=IS_DAYS, oos_days=OOS_DAYS, step=STEP,
                         objective="sharpe")
            if r:
                results[(iname, sname)] = r
                bh_by_index[iname] = r["bh"]
        print(f"{iname:10} div +{DIV_YIELD[iname]:.1f}%/yr | TR-B&H OOS "
    ...
    L = [f"# Multi-index × multi-strategy walk-forward — TOTAL-RETURN benchmark — {today}", "",
    ...
    if sp_tr_real is not None:
        L.append(f"- Real `^SP500TR` full-window CAGR **{sp_tr_real*100:.2f}%** vs ^GSPC price 6.35% "
                 "+ assumed 1.98% div ≈ **8.33%** → the constant-yield approximation reproduces the "
                 "real total-return index. Method sound.")
    L += [f"- Assumed yields (%/yr): " + ", ".join(f"{i} {DIV_YIELD[i]:.1f}" for i in idx_names) +
    ...
    for i in idx_names:
        b = bh_by_index[i]
        L.append(f"| {i} | {b['total_return']*100:+.0f} | {b['sharpe']:.2f} | {b['max_drawdown']*100:.0f} |")

    L += ["", "## OOS Sharpe by strategy × index vs TOTAL-RETURN B&H  (★ beats on return & Sharpe)",
          "", "| strategy | " + " | ".join(idx_names) + " |", "|---|" + "---|" * len(idx_names)]
    ...
    L += ["| _TR-B&H Sharpe_ | " + " | ".join(f"{bh_by_index[i]['sharpe']:.2f}" for i in idx_names) + " |"]
    ...
    for s in ("RSI_reversal", "Bollinger_MR"):
        ...
        if r:
            ftse_note.append(f"FTSE100/{s}: OOS {r['wfa']['total_return']*100:+.0f}% vs TR-B&H "
                             f"{r['bh']['total_return']*100:+.0f}% → beats return: "
                             f"{'YES' if r['beats_bh_ret'] else 'NO'}")

    L += ["", "## Verdict (dividend-adjusted)", "",
    ...
    if ftse_note:
        L += ["- The price-only FTSE 'winners' under dividends: " + "; ".join(ftse_note) +
              " — confirming the earlier price-only outperformance was largely the missing dividend."]
    L += [f"- **Conclusion:** with dividends correctly credited, classic price-timing beats "
    ...
    (REPORTS / f"index_total_return_{today}.json").write_text(json.dumps({
        "date": today, "div_yield": DIV_YIELD, "sp500tr_real_cagr": sp_tr_real,
        ...
        "matrix": {f"{i}|{s}": {"oos_total": r["wfa"]["total_return"], "oos_sharpe": r["wfa"]["sharpe"],
                                "bh_total": r["bh"]["total_return"], "bh_sharpe": r["bh"]["sharpe"],
