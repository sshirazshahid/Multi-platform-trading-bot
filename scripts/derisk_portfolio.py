...
def _plot_portfolio(common, eq_overlay, eq_bh, exposure, eq_sp, today):
    ...
    ax[0].set_title(
        "Buy-and-hold core + de-risk overlay — diversified index portfolio "
        f"({pd.to_datetime(common[0]).year}–{pd.to_datetime(common[-1]).year})"
    )
    ax[0].legend(loc="upper left")
    ax[0].grid(True, which="both", alpha=0.25)
    ax[1].fill_between(
        x, _drawdown(eq_overlay), 0, color="#1f77b4", alpha=0.45, label="Core+overlay"
    )
...
def _plot_per_index_grid(eq_curves, today):
    ...
    fig.suptitle(
        "Per-index: buy-and-hold core + de-risk overlay vs plain B&H (log equity)", fontsize=12
    )
    fig.tight_layout()
...
def main() -> int:
    ...
    pd.DataFrame(
        {
            "date": list(common),
            "basket_buyhold": b_bh_eq.tolist(),
            "basket_core_overlay": b_eq.tolist(),
            "basket_exposure": b_pos.tolist(),
        }
    ).to_csv(REPORTS / f"derisk_portfolio_equity_{today}.csv", index=False)

    # charts (matplotlib optional)
    sp_eq = (
        np.cumprod(1.0 + np.nan_to_num(data["S&P500"].reindex(common).to_numpy()))
        if "S&P500" in data
        else None
    )
    ...
    L += [
        "",
        "## A. Per-index: core + overlay vs buy & hold",
        "",
    ...
    for p in per_index:
        ...
        L.append(
            f"| {p['index']} | {c['cagr'] * 100:+.1f} | {b['cagr'] * 100:+.1f} | {c['sharpe']:.2f} | "
            f"{b['sharpe']:.2f} | {c['max_drawdown'] * 100:.0f} | {b['max_drawdown'] * 100:.0f} | "
            f"{p['avg_exposure'] * 100:.0f}% |"
        )

    dd_better = sum(
        p["core_overlay"]["max_drawdown"] > p["buy_hold"]["max_drawdown"] for p in per_index
    )
    ...
    (REPORTS / f"derisk_portfolio_{today}.json").write_text(
        json.dumps(
            {
                "date": today,
                "params": {"lookback": LOOKBACK, "target_vol": TARGET_VOL, "cap": CAP},
                "basket_core_overlay": basket_overlay_m,
                "basket_buy_hold": basket_bh_m,
                "sp500_buy_hold": sp_bh,
                "basket_avg_exposure": float(np.mean(b_pos)),
    ...
    print(
        f"  per-index overlay improved drawdown {dd_better}/{len(per_index)}, "
