...
def _load_liq() -> dict[str, pd.DataFrame]:
    ...
    with LIQ.open(encoding="utf-8") as f:
        for line in f:
            ...
            by[coin].append(
                (int(r["hour"]), float(r.get("long_usd", 0)), float(r.get("short_usd", 0)))
            )
    out = {}
...
def run(liq_by_coin):
    if not liq_by_coin:
        print(
            "NO liquidation data found at data/liquidations_history.jsonl.\n"
            "Fetch it first (free Coinalyze key, or the WS harvester) — then re-run."
        )
        return
    btc_price = _load_price_1h("BTC")
    if btc_price is None:
        print("BTC 1h price missing from cache — needed for beta adjustment.")
        return
    ...
    if not rows:
        print(
            "No coin had enough overlapping liquidation+price history to screen "
    ...
    for r in sorted(surv, key=lambda x: -x["t"])[:25]:
        print(
            f"  {r['coin']:<8} h={r['h']:<3} {r['hyp']:<20} n={r['n']:<5} "
...
def _selftest():
    ...
    longu = np.abs(rng.normal(1e5, 3e4, n))
    shortu = np.abs(rng.normal(1e5, 3e4, n))
    ...
    liq_by = {
        "SELFCOIN": pd.DataFrame(
            {"hour": hours.astype(int), "long_usd": longu, "short_usd": shortu}
        )
    }
    g = globals()
    orig = g["_load_price_1h"]
    g["_load_price_1h"] = lambda coin: pd.DataFrame(
        {"hour": hours.astype(int), "close": (coin_px if coin == "SELFCOIN" else btc)}
    )
    try:
        print(
            "[selftest] injected long-liq->+1.5% bounce on SELFCOIN; expect H1 t large & positive:"
        )
        run(liq_by)
    finally:
