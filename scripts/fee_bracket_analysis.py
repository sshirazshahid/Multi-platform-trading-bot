...
FEE_TIERS = {
    "current_taker_binance": 0.0005, "current_taker_bybit": 0.0006,
    "futures_maker_binance": 0.0002, "futures_maker_bybit": 0.0001,
    "vip1_taker_example": 0.00035, "zero_fee_floor": 0.0000,
}
BRACKETS = [(0.008,0.013),(0.010,0.018),(0.010,0.010),(0.008,0.020),(0.015,0.015),(0.005,0.015)]

def load_closed(db, mode):
    conn = sqlite3.connect(db); conn.row_factory = sqlite3.Row
    where = "status='CLOSED' AND realized_pnl IS NOT NULL"; params = []
    if mode:
        where += " AND mode=?"; params.append(mode)
    return [dict(r) for r in conn.execute(
        f"SELECT entry_px,size,fee,realized_pnl,market_type,exchange,side FROM trades WHERE {where}", params)]
...
def main():
    ...
    if not Path(args.db).exists():
        print(f"[BLOCKED] warehouse not found: {args.db}"); return 2
    rows = load_closed(args.db, args.mode) or load_closed(args.db, "")
    if not rows:
        print("No closed trades."); return 2
    d = decompose(rows)
    print("=" * 70)
    ...
    for name, leg in FEE_TIERS.items():
        ...
        print(f"    {name:24s} round-trip={leg*2*100:.3f}%  net=${net:8.2f}  (vs recorded {net-d['net_pnl']:+.2f})")
    print(f"  => {'fees are the whole problem' if d['gross_pnl']>0 else 'negative even at ZERO fees - NO fee tier rescues it'}")
    print("\n  --- LEVER 2: bracket breakeven WR vs actual WR ---")
    actual_wr = d["win_rate"]; rt = 0.0022
    print(f"    (actual WR = {actual_wr*100:.1f}%; round-trip cost {rt*100:.2f}%)")
    for sl, tp in BRACKETS:
        noedge = sl/(sl+tp); be = noedge + rt/(sl+tp); gap = actual_wr - be
        ok = "CLEARS" if gap >= 0 else f"short {(-gap)*100:.1f}pp"
        print(f"    SL={sl*100:.1f}% TP={tp*100:.1f}%  no-edge WR={noedge*100:.1f}%  breakeven WR={be*100:.1f}%  -> {ok}")
    print("\nBOTTOM LINE:")
    if d['gross_pnl'] <= 0:
