...
def main() -> None:
    if not KEY:
        print("COINALYZE_API_KEY not set in .env — aborting.")
        return
    ...
    print(
        f"[coinalyze] resolved {len(sym_for)}/{len(COINS)}: "
    ...
    for i in range(0, len(syms), 5):
        ...
        try:
            ...
            print(f"[coinalyze] batch {symstr} failed: {e}")
            continue
                ...
                rows_out.append(
                    {
                        "hour": hour,
                        "symbol": coin,
                        "long_usd": round(longu, 2),
                        "short_usd": round(shortu, 2),
                        "count": 0,
                    }
                )
    ...
    if not rows_out:
        print("[coinalyze] no liquidation rows returned — nothing written.")
        return
    ...
    print(
        f"[coinalyze] wrote {len(rows_out)} rows -> {OUT.name} "
