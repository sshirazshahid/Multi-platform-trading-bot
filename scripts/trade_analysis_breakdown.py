...
_KNOWN_EXITS = {
    "stop_loss", "take_profit", "trailing_stop", "entry_invalidated", "mcp_brain_close",
    "systematic_close", "stale", "ghost_sync", "reconciled_from_exchange", "manual",
    "liquidation", "breakeven", "partial_tp", "time_exit", "daily_loss_lock",
}
...
def group_breakdown(df: pd.DataFrame, by: str, pnl_col: str = "realized_pnl",
                    r_col: str = "r_multiple") -> list[dict]:
    ...
    for g, sub in df.groupby(by, dropna=False, observed=True):
        ...
        avg_r = float(pd.to_numeric(sub[r_col], errors="coerce").dropna().mean()) if r_col in sub else float("nan")
        out.append({
...
def _fmt_rows(rows: list[dict], topn: int | None = None) -> list[str]:
    L = ["| group | n | WR% | sum $ | avg $ | avg R | % of total |",
         "|---|---|---|---|---|---|---|"]
    ...
    for r in show:
        if r.get("group") == "…":
            L.append("| … | | | | | | |")
            continue
        ar = "" if r.get("avg_r") is None or (isinstance(r.get("avg_r"), float) and np.isnan(r["avg_r"])) else f"{r['avg_r']:+.2f}"
        L.append(f"| {r['group']} | {r['n']} | {r['win_rate']*100:.0f} | {r['sum_pnl']:+.2f} | "
...
def main() -> int:
    ...
    attr_tot = {c: float(attr[c].sum()) for c in
                ("alpha", "spread", "slippage", "funding", "fees", "realized_pnl")
                if c in attr.columns} if len(attr) else {}

    # rejected signals
    try:
        rej = pd.read_sql_query(
            "SELECT skip_reason, COUNT(*) n FROM candidates WHERE decision IN ('SKIP','skip','REJECT','reject') "
            "OR skip_reason IS NOT NULL GROUP BY skip_reason ORDER BY n DESC LIMIT 15", con)
    except Exception:
        rej = pd.DataFrame()
    con.close()
    ...
    if attr_tot:
        L += ["", "## Cost / PnL attribution (where the money goes)",
              "| component | total $ |", "|---|---|"]
    ...
    L += ["", "## By exit reason (where PnL leaks)"] + _fmt_rows(group_breakdown(tr, "exit_reason_norm"))
    ...
    L += ["", "_Loss concentrating in one exit reason or a flat score-bucket gradient is the "
    ...
    (REPORTS / f"trade_breakdown_{today}.json").write_text(json.dumps({
        "date": today, "overall": s, "attribution_totals": attr_tot,
    ...
    print(f"Closed trades: {n} | WR {s['win_rate']*100:.1f}% | PnL ${s['total_pnl']:+.2f} | "
    ...
    for r in group_breakdown(tr, "exit_reason_norm"):
        print(f"  {r['group']:18} n={r['n']:>4} WR={r['win_rate']*100:>4.0f}% sum=${r['sum_pnl']:+8.2f} "
