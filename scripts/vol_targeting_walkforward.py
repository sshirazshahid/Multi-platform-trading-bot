...
def main() -> int:
    ...
    for iname, sym in INDICES.items():
        ...
        rows.append({"index": iname, "vt_sharpe": vt["sharpe"], "bh_sharpe": bh["sharpe"],
                     ...
                     "bh_maxdd": bh["max_drawdown"], "d_dd": d_dd,
                     "vt_total": vt["total_return"], "bh_total": bh["total_return"],
                     "wfe": r["wfe_median"], "modal_lookback": r["modal_param"][0]})
        print(f"{iname:10} Sharpe vt {vt['sharpe']:.2f} vs B&H {bh['sharpe']:.2f} (Δ{d_sharpe:+.2f}) | "
