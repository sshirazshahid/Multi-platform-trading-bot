...
def sma_crossover_position(
    close: np.ndarray, fast: int, slow: int, allow_short: bool = False
) -> np.ndarray:
...
def main() -> int:
    ...
    L = [
        f"# Nifty 50 — SMA crossover backtest — {today}",
        "",
    ...
    (REPORTS / f"nifty_sma_backtest_{today}.json").write_text(
        json.dumps(
            {
                "date": today,
                "symbol": SYMBOL,
    ...
    for m in rows:
        print(
            f"  SMA {m['config']:>7}: total {m['total_return'] * 100:+6.1f}%  CAGR "
