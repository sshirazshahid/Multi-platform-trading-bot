...
def main() -> int:
    ...
    for j in range(nC):
        ...
        beta = float(np.cov(rc[m], rb[m])[0, 1] / np.var(rb[m])) if m.sum() > 30 and np.var(rb[m]) > 0 else 1.0
        betas.append(beta)
    ...
    L = [f"# Hyperopt: TRAP vs GATED — {today}", "",
    ...
    if survivors:
        ...
        L += ["**0 survivors.** No parameter set clears the honest gate — exactly what should happen "
    ...
    L += ["## The point", "",
          ...
          "", f"_Betas (IS) per coin: " + ", ".join(f"{c}={b:+.2f}" for c, b in zip(coins, betas)) +
    ...
    (REPORTS / f"hyperopt_demo_{today}.json").write_text(json.dumps({
        "date": today, "n_combos": n_combos,
        "trap": {"best_is_combo": best_is, "is_sharpe": is_sh[best_is], "oos_sharpe": oos_sh[best_is],
                 "is_total_pct": best_is_total_is, "oos_total_pct": best_is_total_oos,
                 "is_oos_rank_corr": float(rho), "pbo": float(pbo_val)},
        "gated": {"oos_fdr_survivors": int(n_fdr), "full_gate_survivors": len(survivors),
                  "survivors": [{"combo": k, "oos_sharpe": sh, "dsr": d} for k, sh, d in survivors]},
    }, indent=2), encoding="utf-8")

    print("\n=== TRAP ===")
    print(f"  IS-best: {best_is}  IS Sharpe={is_sh[best_is]:+.2f} (tot {best_is_total_is:+.1f}%)  "
          f"-> OOS Sharpe={oos_sh[best_is]:+.2f} (tot {best_is_total_oos:+.1f}%)")
    print(f"  family OOS: best={oos_best:+.2f} median={oos_med:+.2f} %positive={frac_oos_pos*100:.0f}%")
    print(f"  IS-vs-OOS rank corr rho={rho:+.3f} | PBO={pbo_val:.2f} (rank-stable but level-negative)")
    print("=== GATED ===")
