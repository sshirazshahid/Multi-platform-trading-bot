...
Mission Control serves a read-only GET surface over the bot's state files
plus a read-only (``mode=ro``) view of ``data/warehouse.sqlite``, and the
operator controls: 9 POST routes under ``/api/ops/*`` (token + operator_note
+ confirm-phrase gated, audited to ``data/mission_control_audit.jsonl``;
CONTROLLED_LIVE stays fail-closed behind the signed checklist).
...
def main() -> int:
    ...
    print(f"Mission Control -> http://{host}:{port}  (token auth required, ops enabled)")
