...
def main() -> int:
    ...
    print(
        f"survivors={result['survivors']}  PBO={result['pbo']}  N_eff={result['n_eff']}  "
    ...
    for r in result["table"][:12]:
        print(
            f"  {r['id']:<16}{r['ir_is']:>8}{r['category']:>14}"
    ...
    print(f"\nReport: {rep}")
    print(
        "CAVEATS: ~14-name universe (low cross-sectional power), ~2-6mo history, "
