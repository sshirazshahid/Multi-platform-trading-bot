...
IR_MIN, DSR_PROMOTE, PBO_MAX, Q, SPLIT, FEE, SLIP, DAYS = (
    0.50,
    0.90,
    0.50,
    0.20,
...
def _funding_series(ex, sym, since):
    ...
    return pd.Series(
        {
            pd.Timestamp(r["timestamp"], unit="ms", tz="UTC").floor("8h"): float(r["fundingRate"])
            for r in fr
        }
    )
...
def fetch_all(syms):
    ...
    for sym in syms:
        ...
        ).pipe(lambda s: s[~s.index.duplicated()])
        for ex2, store in ((by, fund_by), (bg, fund_bg)):
            try:
                ...
                pass
        try:
            oh_oi = bn.fetch_open_interest_history(
                sym, "8h", since=ccxt.binance().milliseconds() - 28 * 24 * 3600 * 1000, limit=500
            )
            oi[sym] = pd.Series(
                {
                    pd.Timestamp(o["timestamp"], unit="ms", tz="UTC").floor("8h"): float(
                        o["openInterestAmount"] or o.get("openInterestValue") or np.nan
                    )
                    for o in oh_oi
                }
            ).pipe(lambda s: s[~s.index.duplicated()])
...
def eval_ls(name, sig, sign, P, h, oos, ntr):
    ...
    return dict(
        ...
        h=h,
        n=int(r.size),
        sharpe=float(sharpe(r.to_numpy())),
        mean=float(r.mean()),
...
def main():
    ...
    print(
        f"panel: {F.shape[0]} bars x {F.shape[1]} syms ({F.index[0]}->{F.index[-1]}) | "
    ...
    diagnostics.append(
        f"B time-series funding IC: mean={ts_ic.mean():+.3f} "
    ...
    if ts_port.size >= 4:
        ...
        rows.append(
            dict(
                ...
                h=1,
                n=int(ts_port.size),
                sharpe=float(sharpe(ts_port.to_numpy())),
                mean=float(ts_port.mean()),
                pval=sharpe_pvalue(ts_port),
                series=ts_port,
                ir_is=float("nan"),
    ...
    for hold in (3, 9, 21):
        ...
        for t in range(len(Fi)):
            ...
            if int(m.sum()) < 10:
                ...
                continue
            idx = np.where(m)[0]
            order = idx[np.argsort(fr_[idx])]
            k = max(1, int(round(int(m.sum()) * Q)))
            lo, sh = order[:k], order[-k:]
            out.append(
                (pr_[lo].mean() - pr_[sh].mean())
                + (fc_[sh].mean() - fc_[lo].mean())
                - cost_per_bar(hold)
            )
    ...
    print("\n" + "=" * 104)
    print(
        f"{'signal':16}{'h':>3}{'IR_is':>8}{'Sharpe':>9}{'mean':>11}{'DSR':>7}{'pval':>7}{'2half':>7}{'FDR':>5}"
    )
    print("-" * 104)
    for r, fdr in zip(rows, fdr_flags):
        ir_s = f"{r['ir_is']:+.3f}" if np.isfinite(r["ir_is"]) else "  n/a"
        print(
            f"{r['name']:16}{r['h']:>3}{ir_s:>8}{r['sharpe']:>9.3f}{r['mean']:>11.5f}"
    ...
    promoted = [
        ...
        if (
            np.isfinite(r["ir_is"])
            and r["ir_is"] >= IR_MIN
            or r["name"].startswith(("B_", "C_", "D_"))
        )
        and r["dsr"] >= DSR_PROMOTE
        and pbo_val <= PBO_MAX
        and fdr
        and r["mean"] > 0
        and r["halves"]
    ...
    print("=" * 104)
    print(
        f"CANDIDATES: {promoted}"
        if promoted
        else "VERDICT: NO_EDGE — nothing cleared the frozen gates across the full sweep."
    )
