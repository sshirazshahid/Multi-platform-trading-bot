...
def main() -> int:
    ...
    for iname, sym in INDICES.items():
        ...
        for sname, (gen, grid) in STRATEGIES.items():
            sbp = {
                f"{sname}{p}": run_backtest(close, gen(close, *p), COST_ONEWAY)["strat_ret"]
                for p in grid
            }
            r = _run_wfa(
                sbp, ret_full, is_days=IS_DAYS, oos_days=OOS_DAYS, step=STEP, objective=OBJECTIVE
            )
            if r:
    ...
    L = [
        f"# Multi-index × multi-strategy walk-forward sweep — {today}",
        "",
    ...
    for i in idx_names:
        b = bh_by_index[i]
        L.append(
            f"| {i} | {spans[i][0]}→{spans[i][1]} | {b['total_return'] * 100:+.0f} | "
            f"{b['sharpe']:.2f} | {b['max_drawdown'] * 100:.0f} |"
        )

    L += [
        "",
        "## OOS Sharpe by strategy × index  (★ = beats buy-and-hold on BOTH return & Sharpe)",
        "",
    ...
    if beat_ret:
        L += [
            "- The only return-beaters — "
            + ", ".join(f"{i}/{s}" for i, s in beat_ret)
    ...
    L += [
        "- **Conclusion:** classic price-timing strategies do **not** produce a robust return edge "
    ...
    (REPORTS / f"index_strategy_sweep_{today}.json").write_text(
        json.dumps(
            {
                "date": today,
                "indices": idx_names,
