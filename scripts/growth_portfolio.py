...
def _plot(curves, dates, today):
    ...
    colors = {
        ...
        "RiskParity": "#2ca02c",
        "RP+VolTarget": "#1f77b4",
        "RP+VolTgt(noLev)": "#17becf",
        "RP+Lever(~SPYvol)": "#ff7f0e",
        "Stk/Bnd 40/60": "#bcbd22",
        "Stk/Bnd 30/70": "#e377c2",
    }
    for name, eq in curves.items():
        ax[0].plot(
            ...
            eq,
            label=name,
            lw=1.8 if name == "RP+VolTarget" else 1.1,
            color=colors.get(name),
            alpha=0.95 if name in ("RP+VolTarget", "SPY") else 0.8,
        )
    ax[0].set_yscale("log")
    ...
    def dd(e):
        e = np.asarray(e)
        return (e / np.maximum.accumulate(e) - 1.0) * 100

    ax[1].plot(x, dd(curves["SPY"]), color="#d62728", lw=1.0, label="SPY")
    ax[1].fill_between(
        x, dd(curves["RP+VolTarget"]), 0, color="#1f77b4", alpha=0.4, label="RP+VolTarget"
    )
...
def main() -> int:
    ...
    runs = {
        "SPY": backtest_portfolio(
            rets, assets, lambda r, t, a: np.eye(len(a))[a.index("SPY")], rebal=10**9
        ),
        ...
        "RP+VolTarget": backtest_portfolio(
            rets, assets, w_inverse_vol, target_vol=TARGET_VOL, cap=3.0
        ),
        "RP+VolTgt(noLev)": backtest_portfolio(
            rets, assets, w_inverse_vol, target_vol=TARGET_VOL, cap=1.0
        ),
        # crux test: lever risk-parity to ~SPY's vol -> does structure beat SPY on RETURN at equal risk?
        "RP+Lever(~SPYvol)": backtest_portfolio(
            rets, assets, w_inverse_vol, target_vol=0.18, cap=3.0
        ),
    }
    ...
    spm = m["SPY"]
    L = [
        f"# Evidence-based growth portfolio — {today}",
        "",
    ...
    for k in order:
        d = m[k]
        L.append(
            f"| {k} | {d['cagr'] * 100:+.1f} | {d['sharpe']:.2f} | {d['sortino']:.2f} | "
    ...
    L += [
        "",
        "## Verdict (robot: maximize risk-adjusted compound growth)",
        "",
        ...
        + (f"; chart: {chart.name}" if chart else "")
        + ". Sensitivity to vol-lookback was robust "
    ...
    (REPORTS / f"growth_portfolio_{today}.json").write_text(
        json.dumps(
            {
                "date": today,
                "assets": assets,
                "window": [str(chart_dates[0]), str(chart_dates[-1])],
                "metrics": {
                    k: {kk: vv for kk, vv in d.items() if kk != "equity"} for k, d in m.items()
                },
                "best_by_sharpe": best,
            },
            indent=2,
    ...
    for k in order:
        d = m[k]
        print(
            f"  {k:18} {d['cagr'] * 100:5.1f}%  {d['sharpe']:5.2f}  {d['max_drawdown'] * 100:5.0f}%  "
