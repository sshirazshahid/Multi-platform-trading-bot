...
Per-bar votes (causal, look-ahead-safe):
  * multifactor: 4h-trend + 1h confirm, score>=65. NOTE: mirrors the VALIDATED
    quant_suite vote, i.e. mcp_brain's pre-2026-06-06 weights (VWAP +15, gate 65,
    structure bonus live). The live brain's standard tier now uses VWAP +5, a
    66/2-bonus gate, and a dead structure bonus — do not quote this as a mirror
    of the current live mcp_brain without re-validating.
  ...
  * funding/carry (Pine #4 port): EMA-smoothed real 8h funding; carry-long when
    funding < -thr in an uptrend, fade-short when funding > +thr in a downtrend.
    Neutral (fc_vote=0) unless a funding frame is supplied to prep().
...
class BTParams:
    ...
    mr_rr: float = 1.5
    rs_len: int = 48
    # funding/carry vote (Pine #4 parity: premThr 0.05%, EMA-8 smooth, ADX>=18)
    fc_thr: float = 0.05
    fc_smooth: int = 8
    fc_adx_min: float = 18.0


def prep(
    df1h: pd.DataFrame,
    df_btc1h: pd.DataFrame | None = None,
    is_benchmark: bool = False,
    p: BTParams | None = None,
    htf_rule: str = "4h",
    ...
    c = d["close"]
    d["ema20"] = ind.ema(c, 20)
    d["ema50"] = ind.ema(c, 50)
    d["rsi"] = ind.rsi(c, 14)
    d["adx"] = ind.adx(d, 14)
    d["atr"] = ind.atr(d, 14)
    ...
    if "dt" in d:
        ...
        o4 = s["open"].resample(htf_rule).first()
        h4 = s["high"].resample(htf_rule).max()
        l4 = s["low"].resample(htf_rule).min()
        c4 = s["close"].resample(htf_rule).last()
        f4 = pd.DataFrame({"open": o4, "high": h4, "low": l4, "close": c4}).dropna()
        if len(f4) > 60:
            E20 = ind.ema(f4["close"], 20)
            E50 = ind.ema(f4["close"], 50)
            E200 = ind.ema(f4["close"], 200)
            A = ind.adx(f4, 14)
            use = (
                pd.DataFrame({"e20": E20, "e50": E50, "e200": E200, "adx": A, "slope": E20.diff(3)})
                .shift(1)
                .reindex(s.index, method="ffill")
            )
            d["ema20_4h"] = use["e20"].values
            d["ema50_4h"] = use["e50"].values
            d["ema200_4h"] = use["e200"].values
            d["adx_4h"] = use["adx"].values
    ...
    if df_btc1h is not None and "dt" in df_btc1h.columns and "dt" in d.columns:
        ...
        baln = bser.reindex(pd.to_datetime(d["dt"], utc=True), method="ffill").reset_index(
            drop=True
        )
    ...
    d["mf_score"] = score
    d["mf_vote"] = np.where(
        req & (score >= 65) & (side4 == 1), 1, np.where(req & (score >= 65) & (side4 == -1), -1, 0)
    )
    ...
    d["mr_vote"] = np.where(
        (c < d["lower"]) & (d["rsi"] < p.mr_rsi_lo) & rs_long,
        1,
        np.where((c > d["upper"]) & (d["rsi"] > p.mr_rsi_hi) & rs_short, -1, 0),
    )

    # ---- funding/carry vote (Pine #4 port; neutral without a funding frame) ----
    # funding: DataFrame {dt: settle time (UTC), rate: decimal per 8h}. Aligned by
    # last settle AT/BEFORE each bar's open (settles land on hour boundaries, so a
    # settle at the bar's open is known during that bar) — causal, like the 4h ctx.
    d["funding_pct"] = np.nan
    d["fc_vote"] = 0
    if funding is not None and len(funding) and "dt" in d.columns:
        fser = (
            funding.set_index(pd.to_datetime(funding["dt"], utc=True))["rate"]
            .astype(float)
            .sort_index()
        )
        aligned = fser.reindex(pd.to_datetime(d["dt"], utc=True), method="ffill")
        d["funding_pct"] = aligned.values * 100.0
        # fund and fc_thr are BOTH in %-per-8h units (rate*100); mixed-cadence
        # symbols (e.g. AXS 1h/4h settles since 2026-01) are NOT re-normalized —
        # their per-unit-time threshold is effectively stricter.
        fund = ind.ema(pd.Series(d["funding_pct"]), p.fc_smooth)
        fc_l = (
            (fund < -p.fc_thr)
            & (c > d["ema20"])
            & (d["ema20"] > d["ema50"])
        ...
        d["fc_vote"] = np.where(fc_l.fillna(False), 1, np.where(fc_s.fillna(False), -1, 0))

    # ---- market regime + ensemble ok flags ----
    trend = d["adx_4h"] >= 25
    d["trend"] = trend
    long_conf = (trend & (d["mf_vote"] == 1) & (d["rg_vote"] == 1)) | (~trend & (d["mr_vote"] == 1))
    short_conf = (trend & (d["mf_vote"] == -1) & (d["rg_vote"] == -1)) | (
        ~trend & (d["mr_vote"] == -1)
    )
...
def _ok_arrays(d: pd.DataFrame, mode: str):
    ...
    if mode == "carry":
        return (d["fc_vote"].values == 1), (d["fc_vote"].values == -1)
    raise ValueError(mode)


def backtest(d: pd.DataFrame, mode: str = "ensemble", p: BTParams | None = None):
    p = p or BTParams()
    ...
    while i < n - 1:
        ...
        if (go_l or go_s) and atr[i] == atr[i] and atr[i] > 0 and o[i + 1] > 0:
            ...
            for j in range(i + 1, min(i + 1 + p.max_hold, n)):
                ...
                if hit_sl:
                    ex = sl * (1 - SLIP_SL) if side == 1 else sl * (1 + SLIP_SL)
                    outc = "SL"
                    break
                if hit_tp:
                    ex = tp * (1 - SLIP_CLOSE) if side == 1 else tp * (1 + SLIP_CLOSE)
                    outc = "TP"
                    break
            if ex is None:
...
def latest_signal(d: pd.DataFrame, mode: str = "confluence", confirmed: bool = True) -> dict:
    ...
    return {
        ...
        "mf_vote": int(row["mf_vote"]),
        "rg_vote": int(row["rg_vote"]),
        "mr_vote": int(row["mr_vote"]),
        "fc_vote": int(row["fc_vote"]),
        "mf_score": float(row["mf_score"]),
        "close": float(row["close"]),
        "atr_pct": float(row["atr"] / row["close"] * 100) if row["close"] else 0.0,
        "rsi": float(row["rsi"]),
        "adx_4h": (float(a4) if a4 == a4 else None),
    }
